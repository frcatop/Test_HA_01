...
This document describes the technical architecture of the Test HA custom component for Home Assistant.

## Directory Structure

```text
custom_components/test_ha/
...
│   ├── __init__.py          # Exports TestHADataUpdateCoordinator
...
│   ├── __init__.py          # Exports TestHAEntity
...
**Key class:** `TestHADataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `TestHAConfigFlowHandler` (main flow)
- `TestHAOptionsFlow` (options)
...
**Key class:** `TestHAEntity` (in `entity/base.py`)
...
2. `TestHAEntity` for common functionality
...
1. Create directory: `custom_components/test_ha/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `TestHAEntity`
