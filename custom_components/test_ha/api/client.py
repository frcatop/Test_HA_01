...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        TestHAApiClientAuthenticationError: For 401 and 403 responses.
        aiohttp.ClientResponseError: For every other unsuccessful status.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise TestHAApiClientAuthenticationError(msg)
    response.raise_for_status()


class TestHAApiClient:
    """
    ...
    async def _api_wrapper(
        ...
        Raises:
            ...
            TestHAApiClientCommunicationError: If the request does not complete.
            TestHAApiClientError: For any other failure.
        ...
        try:
            ...
            raise TestHAApiClientCommunicationError(msg) from exception
        except (aiohttp.ClientError, socket.gaierror) as exception:
            msg = f"Error fetching information - {exception}"
            raise TestHAApiClientCommunicationError(msg) from exception
        except TestHAApiClientError:
            raise
        except Exception as exception:
            msg = f"Unexpected error talking to the API - {exception}"
            raise TestHAApiClientError(msg) from exception
